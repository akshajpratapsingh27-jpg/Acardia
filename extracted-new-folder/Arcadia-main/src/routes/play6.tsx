...
export const Route = createFileRoute("/play6")({
  head: () => ({
    meta: [
      { title: "Play Let's Explore — Find the Pen" },
      {
        name: "description",
        content:
          "Take it slow. Look closely at the room, remember where the pen is hiding, and find it across five gentle rounds.",
      },
      { property: "og:title", content: "Play Let's Explore" },
      {
        property: "og:description",
        content: "Find the hidden pen across five gentle rounds in different rooms.",
      },
    ],
  }),
  component: LetsExploreGame,
});

type Phase = "intro-screen" | "start" | "intro" | "preview" | "hunt" | "found" | "missed" | "end";
    ...
    <main className="relative flex min-h-screen flex-col items-center justify-center px-5 py-8">
      ...
      <section className="relative flex flex-col items-center gap-6 text-center max-w-md">
        ...
        <p className="text-lg text-muted-foreground">
          Take it slow. Little Momo is holding the puzzle for you. Remember where the pen is hiding!
        </p>
        <button
          type="button"
          onClick={onStart}
    ...
    return (
      <main className="bg-warm flex min-h-screen flex-col items-center justify-center px-6 py-10 text-center">
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
          ...
          <Link
            to="/play"
            className="rounded-full px-8 py-4 text-lg font-bold text-muted-foreground hover:text-foreground"
          >
            Back to Games
          </Link>
        </div>
    ...
    <main className="bg-warm flex min-h-screen flex-col items-center px-4 py-6">
      <header className="mb-4 flex w-full max-w-4xl items-center justify-between">
        <div className="flex items-center gap-3">
          <Link to="/play" className="text-sm font-semibold text-muted-foreground hover:text-foreground">
            ← Back
          </Link>
          <div>
...
export const Play6Container = LetsExploreGame;